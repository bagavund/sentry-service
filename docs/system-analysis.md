...
- **Прикладные журналы БД** `GET /api/v1/log?queue=<ключ>` — входящие и сработавшие алерты (обрезаны до `MAX_LOG_ENTRIES`).
...
- **Анти-флуд.** Порог совпадений уже настраивается на очередь; осталось схлопывание — одно сообщение на всплеск плюс редкие обновления «стало +K» вместо сообщения на каждую задачу.
...
Сделано в этой версии: **мультиочередь** (таблица `queues`, `queue_key` во всех таблицах, эндпоинт `/webhook/{queue_key}`, окно/порог/чат на очередь), **админка** `/admin` (вход по паролю, CRUD очередей, общие настройки, вкладка логов, тест уведомлений), **файловый лог с ротацией** (`logging_setup.py`), хранилище настроек в БД (`settings`, приоритет БД → env → умолчание, правки без рестарта), фильтр по очереди на дашборде.
...
`threshold` (совпадений в окне до алерта), `channels` (список тегов, справочно),
`webhook_token` (пусто — общий), `enabled`, `created_at`, `updated_at`.
...
Дедуп-ключ — `queue_key + категория + тег`. Все запросы `tasks`/`seen`/`events`
фильтруются по `queue_key`; окно и порог берутся из записи очереди.
...
**БД → переменная окружения → `DEFAULTS`**. `SQLiteStorage` читает `max_log_entries`,
