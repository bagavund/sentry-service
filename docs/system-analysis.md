...
| Поле | Значение |
| --- | --- |
| Назначение | Раннее обнаружение всплеска однотипных обращений и оповещение техподдержки |
| Статус | Обкатка на одной очереди; мультиочередь и админка готовы к настройке остальных |
...
Клиентская поддержка принимает обращения и заполняет одну из двух форм — «Сайт» или «МП». Из формы создаётся задача в Яндекс Трекере с категорией проблемы, дальше задачу разбирает техническая поддержка. Когда на проде что-то ломается, за несколько минут приходит пачка задач одной категории по одному каналу — это ранний признак инцидента.

**Sentry** непрерывно смотрит на поток новых задач и, как только видит вторую похожую задачу за короткое окно (по умолчанию 30 минут), отправляет сообщение в чат техподдержки в Яндекс Мессенджере: какая категория, какой канал, ссылки на задачи, сколько их уже. Дежурный узнаёт об инциденте за 1–2 минуты, а не тогда, когда очередь вырастет до десятков обращений.

## 2. Проблема

### 2.1. Как устроен поток обращений

1. Клиент обращается в поддержку. Оператор клиентской поддержки заполняет форму обращения. Форм две: одна для веб-сайта, вторая для мобильного приложения.
2. Обращение автоматически превращается в задачу в Яндекс Трекере, которую затем разбирает техническая поддержка.
3. У задачи проставляется **категория** — тип проблемы: «оплата», «каталог не грузится», «ошибка входа» и т. п., и **тег** канала (`Сайт` или `МП`).
...
- техподдержка разбирает задачи по очереди и не сразу видит, что десять последних — про одно и то же;
...
- при превышении порога шлёт текстовый алерт в один чат Яндекс Мессенджера (с кулдауном — не чаще раза в N минут на пару «категория + канал»);
- ведёт короткий журнал входящих событий и сработавших алертов для разбора;
- по запросу показывает, что сейчас в окне дедупа (`GET /api/v1/window`), и умеет «сухой прогон» вебхука без записи (`?dry_run=1`).
...
3. **10:06–10:20** — приходят ещё жалобы. Если у очереди задан кулдаун (`alert_cooldown_minutes`), повторное сообщение по паре «Оплата + Сайт» уйдёт не раньше, чем через это число минут; при `0` — обновление с растущим счётчиком (2, 3, 5, 9…) на каждую новую задачу. Промежуточные дубли в любом случае попадают в журнал и на дашборд.
...
- **Подписка на очередь / штатные уведомления Трекера.** Приходят на каждую задачу по отдельности, без агрегации и без «это уже пятая за десять минут», и не в чат техподдержки AV.
...
- **Требуется до запуска:** развернуть на площадке во внутренней сети, настроить триггер в Трекере, задать `WEBHOOK_TOKEN`, назначить владельца и дежурный чат.
...
```mermaid
flowchart TD
    U["Клиентская поддержка принимает обращение"] --> FORM["Форма: Сайт или МП"]
    FORM --> TR["Задача в Яндекс Трекере (категория + тег канала)"]
    TR --> TRIG["Один триггер на создание задачи"]
...
Всё, кроме файла базы, самого Трекера и мессенджера, — это один процесс в одном контейнере (пакет `app/`). Код разложен по пакету `app/` (модули ниже), статические страницы — в `web/`, разовая утилита — в `tools/`. Роли компонентов:

- **HTTP-слой (`app/main.py`, FastAPI).** Маршрутизация, валидация тела запроса через pydantic, проверка заголовка `X-Webhook-Token`, формирование ответа. Точка запуска — `uvicorn app.main:app`.
- **`DuplicateChecker` (`app/checker.py`, вместе с моделью `TrackerWebhookPayload`).** Оркестрация обработки одной задачи: проверка на повтор, запись в журнал, поиск совпадений, запись в окно, отправка алерта. Держит асинхронный замок для критической секции.
- **`SQLiteStorage` (`app/storage.py`).** Единственная точка доступа к рабочим таблицам (`seen`, `tasks`, `incoming_log`, `duplicate_log`, `events`, `alert_state`), очистку по TTL и обрезку журналов.
- **`YandexMessenger` (`app/messenger.py`).** Формирует текст сообщения и делает HTTP-запрос в API мессенджера. Мягко обрабатывает ошибки — сбой отправки не роняет обработку вебхука.
- **`tools/db_inspect.py`.** Отдельная утилита: печатает содержимое таблиц. В рантайме не участвует, нужна для разбора ситуаций.
- **`web/dashboard.html`.** Самодостаточная страница дашборда, отдаётся маршрутом `GET /dashboard` (раздел 10.3). Внешних ресурсов не тянет.
- **`app/config_store.py` (`ConfigStore`).** Настройки и список очередей в SQLite (таблицы `settings`, `queues`); значение = БД → env → умолчание. Раздел 21.
- **`app/admin.py`.** Роутер `/admin` и `/admin/api/*`: вход по паролю, CRUD очередей, общие настройки, логи. Отдаёт `web/admin.html`.
- **`app/logging_setup.py`.** Единая настройка логирования: консоль + ротируемый файл `sentry.log`. Раздел 16.
...
sequenceDiagram
    ...
    alt да
        ...
        opt N >= порог
            APP->>DB: записать в журнал срабатываний (notified = не на кулдауне)
            opt пара «категория+тег» не на кулдауне
                APP->>MSG: отправить алерт
                MSG-->>APP: ок или ошибка (в лог)
                APP->>DB: отметить время алерта (alert_state)
            end
        end
        APP-->>TR: 200, статус ok, duplicates_found = N, alert_sent
    end
```
...
Тег приходит **из самой задачи**: в поле «Теги» проставлено `Сайт` или `МП` в зависимости от формы. Один триггер на создание задачи передаёт поля задачи (в том числе категорию, область и теги) в тело вебхука как есть — ничего не захардкожено. Сервис использует из этого тела только `category` и `tag`, остальные поля игнорирует.

### 8.2. Правила сравнения

- **Тег задан** (`Сайт` или `МП`) — новая задача сравнивается только с задачами окна, у которых тот же тег.
- **Тег пустой** — новая задача сравнивается со всеми задачами окна этой категории, независимо от тега. Это резервная ветка на случай ручного вызова или если в задаче не проставлен тег канала; при штатной работе теги всегда есть.
...
если len(matches) >= порог:
    first := самая ранняя задача из matches
    на_кулдауне := по паре (queue, category, tag) алерт был за последние alert_cooldown_minutes
    записать запись в duplicate_log (notified = не на_кулдауне)
    если не на_кулдауне:
        отправить алерт в мессенджер (category, tag, новая задача, len(matches), first, ссылки)
        записать время алерта в alert_state

записать строку в events (аналитика — всегда, даже если алерт подавлен)
вернуть {status: ok, duplicates_found: len(matches), duplicate_detected: len(matches) >= порог, alert_sent}
```
...
### 8.6. Порог и кулдаун уведомлений

- **Порог** (`threshold`, на очередь) — сколько совпадений в окне до алерта. По умолчанию `1`: вторая задача с тем же ключом за окно уже даёт алерт.
- **Кулдаун** (`alert_cooldown_minutes`, на очередь; общий дефолт `ALERT_COOLDOWN_MINUTES`). Если по паре «категория + тег» в пределах очереди алерт уже уходил за последние N минут, повторное сообщение в чат **не отправляется**. Промежуточные дубли всё равно пишутся в `duplicate_log` (с флагом `notified: false`) и в `events` — молчит только чат, дашборд остаётся полным. Время последней отправки хранится в таблице `alert_state`.
- **`0` (по умолчанию) — кулдаун выключен**, поведение как раньше: если за окно пришло 5 задач, сообщения уйдут на 2-ю…5-ю, каждое со своим растущим счётчиком.
- Рекомендация: во время долгого инцидента поток из десятков задач превращается в десятки сообщений. Ставьте кулдаун в размер окна (или больше) — одно сообщение на всплеск, детали смотрите на дашборде и через `GET /api/v1/window`.

## 9. Модель данных

Одна база SQLite, путь задаётся `DB_PATH`. Локально — `data/sentry.db`, в контейнере — `/app/data/sentry.db` на именованном томе. Схема создаётся при старте (`CREATE TABLE IF NOT EXISTS`). Лёгкие миграции при старте, идемпотентные: `SQLiteStorage._migrate` добавляет `queue_key` в рабочие таблицы (старые строки → очередь по умолчанию), `ConfigStore._migrate` добавляет `alert_cooldown_minutes` в `queues` (`ALTER TABLE … ADD COLUMN` с проверкой по `PRAGMA table_info`).
...
- `id` (INTEGER, автоинкремент).
- `data` (TEXT) — JSON алерта: новая задача, категория, тег, счётчик, первая задача окна, ссылки, время обнаружения, `notified` (ушло ли сообщение в чат — `false`, если подавлено кулдауном).
- `created_at` (REAL).
...
### 9.5a. Таблица `alert_state` — кулдаун уведомлений

Одна строка на пару, по которой уже слали алерт.

- `queue_key`, `category`, `tag` (TEXT; пустой тег хранится как `''`) — составной первичный ключ.
- `alerted_at` (REAL) — время последней отправки алерта в чат.

Перед отправкой сервис смотрит сюда: если `alerted_at` моложе `alert_cooldown_minutes`, сообщение подавляется. Строки чистятся лениво в `_purge()` — старше суток.

### 9.6. Жизненный цикл записей

- `seen` и `tasks` живут не дольше окна и чистятся лениво при поиске совпадений.
- `alert_state` — строки старше суток удаляются в том же `_purge()`.
...
- `POST /webhook` — приём события от триггера Трекера. Защищён. С `?dry_run=1` — «сухой прогон»: ничего не пишет и не шлёт, возвращает, что *произошло бы* (нормализованные `category`/`tag`, `would_record`, `duplicates_in_window`, `would_alert`). Для настройки триггера.
- `GET /health` — проверка живости, используется в healthcheck контейнера. Открыт.
- `GET /api/v1/stats` — счётчики (журналы, `events_total`, размер окна, порог, кулдаун, retention). Открыт.
- `GET /api/v1/window?queue=` — что сейчас в окне дедупа очереди: группы «категория + тег» со счётчиками, `issue_key`, временем первой/последней задачи. Защищён.
...
Ответ: `200` с телом `{"status": "success", "data": {...}}`, где `data.status` — `ok` или `skipped`. При `ok`: `duplicates_found` (совпадений за окно), `duplicate_detected` (пробит ли порог), `alert_sent` (ушло ли сообщение в чат — `false`, если порог пробит, но сработал кулдаун). При `?dry_run=1` в `data` — прогноз (`would_record`, `would_alert`, …), состояние БД не меняется.

Ошибки: `400` — пустой `issue_key`; `401` — неверный или отсутствующий `X-Webhook-Token` (если защита включена); `404` — очередь не найдена; `403` — очередь отключена.
...
- тело: JSON с полями задачи. `category` и `tag` подставляются из полей задачи (категория и «Теги» со значением `Сайт`/`МП`).
...
Значения `category` и `tag` подставляются из переменных полей задачи (категория очереди и поле «Теги»). Значение тега — `Сайт` или `МП` — приходит из самой задачи, в триггере не фиксируется.
...
Ранее алерты уходили в Telegram (модуль `telegram_bot.py`). Модуль удалён и заменён на `app/messenger.py` с тем же набором методов. Эндпоинт проверки переименован с `/api/v1/test-telegram` на `/api/v1/test-notify`.
...
**Мультиочередь и админка**

- `ADMIN_PASSWORD` — пароль входа в `/admin`. Пусто — админка отключена. Можно сменить в самой админке.
- `DEFAULT_QUEUE_KEY` — ключ очереди для legacy `POST /webhook`. По умолчанию `default`.
- `ALERT_THRESHOLD` — стартовый порог совпадений для очереди по умолчанию. По умолчанию `1`.
- `ALERT_COOLDOWN_MINUTES` — стартовый кулдаун уведомлений (минут) для очереди по умолчанию и формы новой очереди. По умолчанию `0` (выключен).

**Логирование**

- `LOG_LEVEL` — `DEBUG`/`INFO`/`WARNING`/`ERROR`. По умолчанию `INFO`. Меняется в админке на лету.
- `LOG_DIR` — каталог файла лога. Пусто — каталог файла БД.
- `LOG_FILE` — имя файла. По умолчанию `sentry.log`.
- `LOG_MAX_BYTES` / `LOG_BACKUP_COUNT` — ротация. По умолчанию `5242880` (5 МБ) и `5`.
...
После первого старта значения (кроме `ADMIN_PASSWORD`, `DB_PATH`, `PORT`, `LOG_*`, которые нужны до инициализации) переопределяются из админки и хранятся в таблице `settings`.
...
Короткие команды в `Makefile`: `make up`, `make down`, `make logs`, `make rebuild`, `make token` (генерация значения для `WEBHOOK_TOKEN`), `make test` (pytest), `make run` (локальный запуск `python -m app.main`).
...
- В контейнере: `docker compose exec sentry python tools/db_inspect.py` — печатает счётчики и последние записи всех таблиц, включая рабочее окно `tasks`, `seen` и `events`.
...
- **Алертов слишком много.** Задать очереди `alert_cooldown_minutes` (в админке, вкладка «Очереди») — тогда по одной паре «категория + тег» сообщение уходит не чаще раза в N минут. Значение порядка размера окна убирает поток, не теряя первый сигнал.
- **Сомнения «почему сработало / почему нет».** `GET /api/v1/window?queue=<ключ>` (или `tools/db_inspect.py`) — фактическое содержимое окна. `POST /webhook?dry_run=1` с тем же телом — покажет, как сервис разобрал `category`/`tag` и что решил бы.
- **После перезапуска сомнения в состоянии.** `tools/db_inspect.py` / `GET /api/v1/window` покажут, что в окне; при необходимости `POST /api/v1/clear`.
- **Контейнер в рестарт-петле.** Смотреть `docker compose logs`. Вероятные причины: недоступен путь тома, битый `.env`.

## 16. Наблюдаемость

- **Логи процесса** (уровень `LOG_LEVEL`, по умолчанию INFO): каждое входящее событие с ключом очереди, пропуск повтора, срабатывание с числом совпадений, результат отправки в мессенджер, вход в админку и правки настроек, предупреждение об открытых эндпоинтах при пустом токене.
  - **stdout** — собирается Docker (`docker compose logs -f`, ротация 3 × 10 МБ).
  - **Файл** `${LOG_DIR|каталог БД}/sentry.log` (в Docker `/app/data/sentry.log`, тот же том, переживает рестарт). Ротация `LOG_MAX_BYTES` × `LOG_BACKUP_COUNT` (по умолчанию 5 МБ × 5). Настройка — модуль `logging_setup.py`.
  - Просмотр: вкладка «Логи» в `/admin`, либо `GET /api/v1/logs?lines=N` (под токеном).
- **Прикладные журналы БД** `GET /api/v1/log?queue=<ключ>` — входящие и сработавшие алерты (обрезаны до `MAX_LOG_ENTRIES`); у записей `duplicate_log` есть `notified` — ушло сообщение в чат или подавлено кулдауном.
- **Окно дедупа** `GET /api/v1/window?queue=<ключ>` — что накоплено за окно прямо сейчас.
- **Дашборд** `/dashboard` (с фильтром по очереди) и `GET /api/v1/stats` (`events_total`, счётчики по очередям).
...
`pytest` в каталоге `tests/` (зависимости — `requirements-dev.txt`, конфиг — `pytest.ini`):
...
Прогон изолирован: `tests/conftest.py` уводит `DB_PATH` и файл лога во временный каталог, отключает сеть и мессенджер (пустые токены). Покрытие:

- `test_config_store.py` — приоритет БД → env → умолчание, `get_int/get_bool`, нераскрытие секретов, сид очереди по умолчанию, валидация ключа, частичное обновление очереди, `alert_cooldown_minutes`, `max_window_minutes`, приоритет токена очереди, стабильность секрета сессии, миграция старой БД (без `alert_cooldown_minutes`).
- `test_storage.py` — `seen` TTL, дедуп по «категория + тег» и по очереди, границы окна, обрезка журналов до `MAX_LOG_ENTRIES`, `alert_state` (кулдаун, протухание, другой тег), `window_snapshot`, агрегаты аналитики (`summary`, `by_category`, `by_channel`, `heatmap`), очистка по очереди.
- `test_duplicate_checker.py` — первая задача молчит / вторая шлёт, порог `threshold`, изоляция по тегу, `skipped` на повторе `issue_key`, **подавление повторных алертов кулдауном** (в чат ушло 1 сообщение, в журнал — 3, в `events` — 4), `dry_run` ничего не пишет.
- `test_api.py` — `/health`, `/`, вебхук и дедуп через HTTP, `404`/`403` для очереди, обязательность `X-Webhook-Token`, `dry_run`, `/api/v1/window` (+ токен), `/api/v1/stats`, `/api/v1/analytics/overview`, `/dashboard`, `/api/v1/clear` под токеном, вход в админку, CRUD очередей, запрет удаления очереди по умолчанию, круговой рейс общих настроек.
- `test_messenger.py` — отключён без токена (только консоль), сборка запроса (URL, тело, заголовок `Bearer`), `401`/HTTP-ошибка → «не отправлено», содержимое текста уведомления. `requests.Session` подменяется — сети нет.
- `test_logging_setup.py` — идемпотентность обработчиков, `watchfiles` приглушён до `WARNING`, uvicorn-логгеры проксируются в корневой, `tail_log`.

### 17.2. Ручная проверка на целевой площадке

То, что автотесты не покрывают (нужна внутренняя сеть AV):

1. `POST /api/v1/test-notify` — реальная доставка в чат Яндекс Мессенджера.
2. Триггер в Трекере действительно вызывает `/webhook` с корректными `category`/`tag` (проверить `POST /webhook?dry_run=1` с боевым телом).
3. `/dashboard` открывается и рисует графики по отправленным задачам.

## 18. Дорожная карта

- **Схлопывание алертов.** Кулдаун уже гасит поток; следующий шаг — редкие агрегированные обновления «за N минут стало +K» вместо полного молчания на время кулдауна.
...
- **Аудит правок в админке** — сейчас изменения только пишутся в лог, отдельной таблицы истории «кто и когда менял» нет.
- **Учётные записи админки** — сейчас один общий пароль; при необходимости — таблица пользователей.
- **Вынести `events`** в отдельное хранилище или BI-приёмник, если появится очередь с тысячами задач в сутки.
- **Ручной mute/resolve** всплеска из чата или админки.

Сделано в этой версии: **кулдаун уведомлений** (`alert_cooldown_minutes` на очередь, таблица `alert_state`, флаг `notified` в `duplicate_log`), **`GET /api/v1/window`** (снимок окна дедупа), **`POST /webhook?dry_run=1`** (сухой прогон), **автотесты** (`pytest`, каталог `tests/`, ~55 кейсов). Ранее: мультиочередь, админка `/admin`, файловый лог с ротацией, хранилище настроек в БД, фильтр по очереди на дашборде.
...
- **Шум при инциденте.** Кулдаун (`alert_cooldown_minutes`) гасит поток, но по умолчанию выключен (`0`); без него — сообщение на каждую задачу. Полного схлопывания с агрегированными обновлениями пока нет.
...
- **Автотесты не покрывают** реальную отправку в мессенджер и приём вебхука из Трекера — это проверяется вручную на площадке (раздел 17.2).

## 20. Глоссарий

- **Категория** — тип проблемы в обращении клиента; поле задачи Трекера.
- **Тег / канал** — источник обращения: `Сайт` или `МП`. Проставляется в поле «Теги» задачи и приходит в вебхуке из её данных.
...
## 21. Мультиочередь и админка

### 21.1. Модель очереди

Очередь (`queues`) — единица настройки. Поля: `key` (в URL вебхука, неизменяем),
`title`, `chat_id`, `messenger_token` (пусто — берётся общий), `window_minutes`,
`threshold` (совпадений в окне до алерта), `alert_cooldown_minutes` (не слать
повторный алерт по паре «категория + тег» чаще раза в N минут; `0` — выкл),
`channels` (список тегов, справочно), `webhook_token` (пусто — общий), `enabled`,
`created_at`, `updated_at`.

При первом старте, если очередей нет, создаётся одна с ключом `DEFAULT_QUEUE_KEY`
и `chat_id` из `YANDEX_MESSENGER_CHAT_ID` — поведение прежней однопоточной версии
сохраняется без действий оператора.

### 21.2. Маршрутизация вебхука

- `POST /webhook/{queue_key}` — явный выбор очереди.
- `POST /webhook` (legacy) — очередь из тела (`"queue"`), иначе `DEFAULT_QUEUE_KEY`.
- Очередь не найдена → `404`, отключена → `403`.
- Токен: у очереди свой (`webhook_token`), иначе общий (`WEBHOOK_TOKEN`); пусто — приём открыт.

Дедуп-ключ — `queue_key + категория + тег`. Все запросы `tasks`/`seen`/`events`/`alert_state`
фильтруются по `queue_key`; окно, порог и кулдаун берутся из записи очереди.

### 21.3. Хранилище настроек

`config_store.py::ConfigStore` — отдельное соединение к тому же SQLite (WAL
допускает несколько соединений; запись идёт только из админки). Таблица `settings`
(key-value) кэшируется в память, инвалидируется при записи. Значение настройки:
**БД → переменная окружения → `DEFAULTS`**. При старте `_migrate()` добирает
недостающие колонки `queues` (`ALTER TABLE … ADD COLUMN` с проверкой по
`PRAGMA table_info`) — идемпотентно, безопасно для старых баз. `SQLiteStorage` читает `max_log_entries`,
`timezone_offset`, `events_retention_days` через свойства — правки применяются без
рестарта. Секреты (`*_token`, `admin_password`, `admin_session_secret`) наружу не
отдаются — только признак «задано».

### 21.4. Админка

- `GET /admin` — одна самодостаточная страница `web/admin.html` (без CDN), три вкладки:
  очереди, общие настройки, логи.
- `/admin/api/*` — JSON. Аутентификация: один пароль `admin_password`
  (`ADMIN_PASSWORD` или правится в самой админке). Сессия — кука `sentry_admin`
  вида `<issued_at>.<hmac_sha256>`, секрет `admin_session_secret` генерируется и
  хранится в `settings`, TTL 7 дней. Пароль пуст → вход отключён (`503` на `/login`).
- Действия: CRUD очередей, тест уведомления (`messenger_for(queue).send_test_message()`),
  очистка данных очереди, правка общих настроек, хвост лог-файла. Все изменения
  пишутся в лог (`sentry.admin`).
- Удаление очереди по умолчанию запрещено (сначала назначить другую); удаление
  единственной очереди запрещено; при удалении данные очереди чистятся.

### 21.5. Ограничения

- Общий пароль без учёток и без таблицы аудита правок (только запись в лог).
- Кука не привязана к IP/устройству; при HTTP (внутренняя сеть) флаг `secure` не ставится.
- `ConfigStore` держит второе соединение к файлу БД — на текущей нагрузке безопасно,
  но формально отходит от инварианта «одно соединение под одним замком».

## Приложение А. История изменений архитектуры

- **01.09.2026 (раскладка по пакету).** Плоский корень разложен: код — в пакет `app/` (`main.py` — HTTP-слой, `storage.py` — `SQLiteStorage`, `checker.py` — `DuplicateChecker` + `TrackerWebhookPayload`, плюс `config_store.py`, `admin.py`, `messenger.py`, `logging_setup.py`), HTML — в `web/`, `db_inspect.py` — в `tools/`. Точка запуска `app:app` → `app.main:app` (`Dockerfile`, локально `python -m app.main`). Импорты внутри — абсолютные (`from app.xxx import …`). Заодно убран мёртвый код: `YandexMessenger.send_error_message`, `ConfigStore.get_bool`, env-fallback'и в мессенджере; проверки токена вебхука сведены в одну функцию. Логгер бизнес-логики `sentry.app` → `sentry.checker`. Поведение и схема БД не менялись; `pytest` — 54 кейса, зелёные.
