...
- **`SQLiteStorage`.** Единственная точка доступа к рабочим таблицам (`seen`, `tasks`, `incoming_log`, `duplicate_log`, `events`, `alert_state`), очистку по TTL и обрезку журналов.
...
- **`config_store.py` (`ConfigStore`).** Настройки и список очередей в SQLite (таблицы `settings`, `queues`); значение = БД → env → умолчание. Раздел 21.
- **`admin.py`.** Роутер `/admin` и `/admin/api/*`: вход по паролю, CRUD очередей, общие настройки, логи. Отдаёт `admin.html`.
- **`logging_setup.py`.** Единая настройка логирования: консоль + ротируемый файл `sentry.log`. Раздел 16.
...
- **Сомнения «почему сработало / почему нет».** `GET /api/v1/window?queue=<ключ>` (или `db_inspect.py`) — фактическое содержимое окна. `POST /webhook?dry_run=1` с тем же телом — покажет, как сервис разобрал `category`/`tag` и что решил бы.
- **После перезапуска сомнения в состоянии.** `db_inspect.py` / `GET /api/v1/window` покажут, что в окне; при необходимости `POST /api/v1/clear`.
...
- `GET /admin` — одна самодостаточная страница `admin.html` (без CDN), три вкладки:
...
- **31.08.2026.** Redis и старый дашборд убраны как избыточные. Состояние переведено в SQLite. Разделение каналов — по тегу задачи (поле «Теги» со значением `Сайт`/`МП`), который триггер передаёт в теле вебхука. Канал уведомлений — с Telegram на Яндекс Мессенджер. Добавлена утилита `db_inspect.py`.
...
- **01.09.2026 (кулдаун, наблюдаемость, автотесты).** Кулдаун уведомлений: поле очереди `alert_cooldown_minutes` (общий дефолт `ALERT_COOLDOWN_MINUTES`), таблица `alert_state`, флаг `notified` в `duplicate_log`, поле `alert_sent` в ответе вебхука — при пробитом пороге повторный алерт по паре «категория + тег» подавляется на N минут, аналитика при этом фиксирует всё. Новые ручки: `GET /api/v1/window` (снимок окна дедупа, под токеном) и `POST /webhook?dry_run=1` (сухой прогон без записи). `ConfigStore._migrate()` (`ALTER TABLE queues`). Каталог `tests/` — `pytest`-набор (~55 кейсов) на `ConfigStore`, `SQLiteStorage`, `DuplicateChecker`, HTTP-слой, `YandexMessenger`, `logging_setup`; `requirements-dev.txt`, `pytest.ini`. Ранее (31.08) исправлен шторм `watchfiles` в режиме `--reload` (`logging_setup.py` глушит логгер до `WARNING`, `reload_excludes` в `app.py`).
- **31.08.2026 (v2 — мультиочередь и админка).** Добавлены модули `config_store.py` (таблицы `settings`, `queues`; приоритет БД → env → умолчание), `admin.py` (роутер `/admin` + `/admin/api/*`, вход по паролю через подписанную HMAC-куку), `logging_setup.py` (консоль + ротируемый файл `sentry.log`), страница `admin.html`. В `seen`/`tasks`/`incoming_log`/`duplicate_log`/`events` добавлена колонка `queue_key` (лёгкая миграция `ALTER TABLE` при старте, старые строки → очередь по умолчанию). Новые эндпоинты: `POST /webhook/{queue_key}`, `GET /api/v1/queues`, `GET /api/v1/logs`, `?queue=` у аналитики/логов/clear/test-notify. `SQLiteStorage` и `DuplicateChecker` берут параметры из `ConfigStore` на лету; окно и порог — на очередь. Дашборд получил селектор очереди. Версия FastAPI-приложения — `2.0.0`.
