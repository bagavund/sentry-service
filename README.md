...
Теги `Сайт` и `МП` разделяют две формы обращений (сайт / мобильное приложение),
...
Триггер на создание задачи → действие **«Вызвать HTTP»**:
...
`tag` проще захардкодить в каждом триггере: у вас две формы → два триггера,
в одном `"Сайт"`, в другом `"МП"`. `{{issue.category}}` замените на реальную
переменную вашего поля категории.
